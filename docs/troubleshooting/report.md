...
ログの提出を求められたときは、Unityの`Player.log`を確認します。通常のWindows環境では`%USERPROFILE%\AppData\LocalLow\feath\OVR_DummyHeadMic\Player.log`にあります。デバイス名やローカル環境の情報が含まれる可能性があるため、公開のIssueへそのまま添付する前に内容を確認してください。
