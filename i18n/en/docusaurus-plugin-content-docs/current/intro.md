---
sidebar_position: 1
slug: /intro
---

# OVR-DummyHeadMic User Guide

OVR-DummyHeadMic is a Windows SteamVR overlay application that places a virtual dummy head microphone in VR and converts your voice to binaural audio in real time. It lets VR streams and recordings reflect the position of the speaker relative to the microphone.

This guide explains how to configure audio devices, route audio to OBS, place the dummy head, adjust spatial audio, synchronize with the VRChat camera, and troubleshoot issues.

## Choose what you want to do

- [Check the requirements](getting-started/requirements.md)
- [Install or update](getting-started/install-update.md)
- [Play your first sound](getting-started/first-sound.md)
- [Choose an audio route](audio/routes.md)
- [Use a DAW with the VST bridge](audio/vst-bridge.md)
- [Route audio to OBS](audio/obs.md)
- [Place the dummy head](operation/dummy-head.md)
- [Look up a setting](operation/settings.md)
- [Synchronize with the VRChat camera](vrchat-camera/overview.md)
- [Troubleshoot an issue](troubleshooting/common-issues.md)

:::info
If this is your first time using the app, start with “Requirements” and follow the pages in order. First verify the audio with manual placement, then set up VRChat camera sync. This makes it easier to identify problems.
:::
